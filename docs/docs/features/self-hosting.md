...
Local directory layout next to your `.env`: the GitHub App `<key>.pem` (own-App mode only), plus `data/` (SQLite database), `logs/`, and `repos/` (clones and worktrees). This works for both local workstation setup and remote server deployment. See [Setup](../tutorials/setup.md) for the full flow, including the required GitHub App permissions (Contents R/W, Issues R/W, Pull Requests R/W, Metadata R). Add **Actions: write** only for repositories where you enable `/deploy`; Actions read is used for CI check results.
