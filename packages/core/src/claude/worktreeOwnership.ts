...
import { resolveGitCommonDir, resolveLinkedWorktreeGitDir } from '../git/worktreeOperations.js';
...
export async function setWorktreeOwnership(
    ...
    try {
        ...
        try {
            ownershipTargets.push(await resolveGitCommonDir(worktreePath));
        } catch (commonDirError) {
            logger.debug({ issueNumber, worktreePath, error: (commonDirError as Error).message }, 'Could not resolve shared Git metadata before agent launch');
        }
